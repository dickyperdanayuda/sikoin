<?php
class Model_LaporanKwitansi extends CI_Model
{
    ...
    private function _get_datatables_query($tgl1, $tgl2)
    {
        ...
        if ($tgl1 != 'null' and $tgl2 != 'null') {
            $this->db->where("pkw_tgl >= '{$tgl1}'");
            $this->db->where("pkw_tgl <= '{$tgl2}'");
        }
        $i = 0;
    ...
    public function get_terpakai($id)
    {
        $this->db->select("kw_pemegang");
        $this->db->from("ktk_kwitansi");
        $this->db->join("ktk_pengambilan_kwitansi", "pkw_kry_id = kw_pemegang", "left");
        $this->db->where("kw_pemegang", $id);
        $this->db->where("kw_don_id !=''");
        $query = $this->db->get();
