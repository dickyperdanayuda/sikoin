...
class Login extends CI_Controller
{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($cek->num_rows() > 0) {
				...
				foreach ($cek->result() as $qad) {
					// $nama = "";
					// $peg = $this->Model_Login->get_pegawai($qad->log_peg_id);
					...
					$sess_data['username'] = $qad->log_username;
					$sess_data['password'] = $qad->log_password;
					// $sess_data['id_peg'] = $qad->log_peg_id;
					// $sess_data['id_jab'] = $peg->peg_jab_id;
					$sess_data['nama'] = $usr;
					$sess_data['level'] = $qad->log_level;
					$sess_data['log_kry'] = $qad->log_kry_id;
	...
	public function ubah_pass()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($cek->num_rows() > 0) {
				$data = array(
					'log_password' => md5($this->input->post('log_passBaru'))
				);
				$up_pass = $this->Model_Login->update('ktk_login', array('log_username' => $u, 'log_password' => md5($p)), $data);
				if ($up_pass >= 0) {
