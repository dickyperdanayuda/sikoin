...
class LaporanBlmValidasi extends CI_Controller
{
    ...
    public function ajax_list_laporan($tgl1, $tgl2)
    {
        ...
        foreach ($list as $laporan) {
            $no++;
            if (($laporan->jpt_tgl_hitung != null) && ($laporan->jpt_tgl_validasi == null)) {
                $row = array();
                $row[] = $no;
                $row[] = $laporan->jpt_tgl_validasi;
